...
import 'package:altogic_dart/utils/model/handle_request_result.dart';
...
class Fetcher {
  ...
  Future<HandleRequestResult> _handleRequest(
  String method,
  ...
  // resolveType: "json" | "text" | "blob" | "arraybuffer" = "json"
    ...
    if (path.isEmpty || !path.trim().startsWith("/")) {
...
return this._handleRequest("GET", path, query, headers, null, resolveType);
}
...
return this._handleRequest("POST", path, query, headers, body, resolveType);
}
...
return this._handleRequest("PUT", path, query, headers, body, resolveType);
}
...
this._headers.Session = session.token;
}
}
